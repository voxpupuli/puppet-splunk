          ...
          context 'splunk class with legacy_mode = false and boot_start = true (defaults)' do
            ...
            if facts[:kernel] == 'Linux' || facts[:kernel] == 'SunOS'
              context 'with service_provider init' do
                ...
                it { is_expected.not_to contain_service('splunkweb') }
                it { is_expected.not_to contain_service('Splunkd') }
              end
          ...
          context 'splunk class with legacy_mode = false and boot_start = false' do
            ...
            if facts[:kernel] == 'Linux' || facts[:kernel] == 'SunOS'
              context 'with service_provider init' do
                ...
                it { is_expected.not_to contain_service('Splunkd') }
                it {
                  is_expected.to contain_service('splunk').with(restart: '/opt/splunk/bin/splunk restart',
              ...
              context 'splunk::version >= 7.2.2 with service_provider systemd' do
                ...
                it { is_expected.not_to contain_service('splunkweb') }
                it {
                  is_expected.to contain_service('Splunkd').with(restart: '/opt/splunk/bin/splunk restart',
                                                                 start: '/opt/splunk/bin/splunk start',
                                                                 stop: '/opt/splunk/bin/splunk stop',
                                                                 provider: 'base')
                }
          ...
          context 'splunk class with legacy_mode = true and boot_start = false' do
            ...
            if facts[:kernel] == 'Linux' || facts[:kernel] == 'SunOS'
              context 'with service_provider init' do
                ...
                it { is_expected.not_to contain_service('Splunkd') }
                it {
                  is_expected.to contain_service('splunkd').with(restart: '/opt/splunk/bin/splunk restart splunkd',
                                                                 start: '/opt/splunk/bin/splunk start splunkd',
                                                                 stop: '/opt/splunk/bin/splunk stop splunkd',
                                                                 provider: 'base')
                }
                it {
                  is_expected.to contain_service('splunkweb').with(restart: '/opt/splunk/bin/splunk restart splunkweb',
